...
export default function QuestionCard({
  ...
  const { setScorePageVisibility, userScores } = useQuiz();
    ...
    <>
      <form onSubmit={submitHandler} className="form-container">
        <div className="quiz-question-options-parent-container">
          <div className="question-progress-bar-container">
            <p className="question-indicator">
              Question {currentQuestionIndex + 1} of {noOfQuizQuestions}
            </p>
            <p className="question">{question}</p>
            <ProgressBar
              currentQuestionIndex={currentQuestionIndex}
              noOfQuizQuestions={noOfQuizQuestions}
              className="quiz-container-progress-bar"
            />
          </div>
        ...
        <div className="submit-button-container">
          {showCorrectAnswer ? (
            <FormButton className="submit-answer-button" type="submit">
              Next Question
            </FormButton>
          ) : (
            <FormButton
        ...
        {showWarning && !checkedValue && (
          <div className="warning-container">Please Select a Option</div>
        )}
      </form>
    </>
  );
