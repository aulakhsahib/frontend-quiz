...
export default function RadioButton({ name, value, checkedValue, setCheckedValue, children, defaultBtn, selected, correctSelected, incorrectSelected, correctDidNotSelected, disableBtn}) {
    const classesToAdd = clsx(
        'default-button',
        {
            '': defaultBtn,
    ...
    return (
        <div>
            <label className={classesToAdd}>
                ...
                <input type="radio" name={name} value={value} checked={checkedValue === value} onChange={(e) => setCheckedValue(e.target.value) } disabled={disableBtn}/>
            </label>
        </div>
    );
}
